...
export default function Profile() {
  ...
  // Active sub-tab inside Profile page: 'loans' (My Listings/Offers) or 'offers' (Sent Offers)
  const [activeTab, setActiveTab] = useState('loans');

  // Fetch Listings / Loan Offers created by this user
  ...
  // Fetch Offers Sent on other listings
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 flex flex-col text-center md:text-left">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-center md:justify-start">
            ...
            <div className="flex items-center gap-2.5 justify-center md:justify-start">
              <button
                ...
                className="px-4 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/95 transition-all shadow-sm hover:shadow cursor-pointer inline-flex items-center gap-1.5"
              >
                <PlusCircle size={14} />
                <span>{user.role === 'lender' ? 'Create Loan Offer' : 'Create Loan Request'}</span>
              </button>
              <Link
          ...
          <div className="hidden sm:flex items-center gap-8 text-sm text-muted-foreground my-4">
            <span>
              <strong className="text-foreground font-semibold">{myLoans.length}</strong> {user.role === 'lender' ? 'loan offers created' : 'loan requests'}
            </span>
            <span>
      ...
      <div className="sm:hidden flex items-center justify-around text-center text-xs text-muted-foreground border-y border-border/60 py-3 mt-4">
        <div>
          <strong className="block text-foreground font-semibold text-sm">{myLoans.length}</strong> {user.role === 'lender' ? 'offers created' : 'loan requests'}
        </div>
        <div>
      ...
      <motion.div variants={itemVariants} className="flex justify-center border-t border-border/60 mt-4 sm:mt-8">
        <div className="flex gap-8 sm:gap-12 -mt-[1.5px]">
          <button
            onClick={() => setActiveTab('loans')}
            className={`flex items-center gap-1.5 pt-4 pb-2 text-[11px] font-bold uppercase tracking-widest transition-all cursor-pointer ${activeTab === 'loans'
                ? 'border-t-2 border-foreground text-foreground'
                : 'border-t border-transparent text-muted-foreground hover:text-foreground'
              }`}
          >
            <Banknote size={14} />
            <span>{user.role === 'lender' ? 'My Loan Offers' : 'My Listings'}</span>
          </button>
          {isLender && (
            <button
              onClick={() => setActiveTab('offers')}
              className={`flex items-center gap-1.5 pt-4 pb-2 text-[11px] font-bold uppercase tracking-widest transition-all cursor-pointer ${activeTab === 'offers'
                  ? 'border-t-2 border-foreground text-foreground'
                  : 'border-t border-transparent text-muted-foreground hover:text-foreground'
                }`}
            >
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-extrabold text-foreground tracking-tight">
                {user.role === 'lender' ? 'Your Capital / Loan Offers' : 'Your Loan Listings'}
              </h2>
              <span className="text-xs text-muted-foreground font-semibold">{myLoans.length} total</span>
            </div>
            ...
            {loansLoading ? (
              ...
              <div className="text-center py-16 rounded-2xl border border-border border-dashed bg-card space-y-3">
                <Banknote size={40} className="mx-auto text-muted-foreground/60 mb-1" />
                <p className="text-base font-bold text-foreground">
                  {user.role === 'lender' ? 'No loan offers created yet' : 'No loan requests created yet'}
                </p>
                <p className="text-sm text-muted-foreground max-w-sm mx-auto">
                  {user.role === 'lender'
                    ? 'Provide capital to borrowers by creating a verified loan offer on the marketplace.'
                    : 'Need money? Create a loan request to get matching funding offers from verified lenders.'}
                </p>
                <button
                  onClick={() => setCreateModalOpen(true)}
                  className="px-4 py-2 rounded-xl bg-primary text-primary-foreground text-xs font-bold hover:bg-primary/95 transition-all shadow-xs cursor-pointer inline-flex items-center gap-1.5"
                >
                  <PlusCircle size={14} />
                  <span>{user.role === 'lender' ? 'Create Loan Offer' : 'Create Loan Request'}</span>
                </button>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wide border ${loan.listingType === 'lend'
                                    ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20'
                                    : 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20'
                                    }`}
                                >
                                  {loan.listingType === 'lend' ? 'Lender Offer' : 'Borrow Request'}
                                </span>
                              </div>
              ...
              <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-border/50 bg-card shrink-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="font-extrabold text-foreground text-base sm:text-lg">
                    {user.role === 'lender' ? 'Create Loan Offer (Provide Capital)' : 'Create Loan Listing'}
                  </h3>
                </div>
                <button
            ...
            >
              <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-border/50 bg-card shrink-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="font-extrabold text-foreground text-base sm:text-lg">
                    {selectedLoan.listingType === 'lend' ? 'Edit Loan Offer' : 'Edit Loan Request'}
                  </h3>
                </div>
                <button
...
function LoanRequestForm({ accessToken, loan, onCreated, onCancel, userRole }) {
  const isEditing = !!loan;

  const [form, setForm] = useState({
    listingType: loan?.listingType || (userRole === 'lender' ? 'lend' : 'borrow'),
    amount: loan?.amount || '',
    ...
    <>
      ...
      <form onSubmit={submit} className="flex flex-col gap-4 max-h-[65vh] overflow-y-auto pr-1">
        ...
        <InfoBanner variant="info" dismissible={true} storageKey="mt_borrower_disclaimer_seen">
          MT Pocket never handles your money — all transactions occur directly between verified peers.
        </InfoBanner>

        {/* 2-Way Listing Type Selector */}
        <div>
          <label className={labelCls}>Listing Type *</label>
          <div className="grid grid-cols-2 gap-2 p-1 bg-muted/40 rounded-2xl border border-border/70">
            <button
              type="button"
              onClick={() => set('listingType', 'borrow')}
              className={`py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                form.listingType === 'borrow'
                  ? 'bg-card text-emerald-600 dark:text-emerald-400 shadow-xs border border-emerald-500/30 font-extrabold'
                  : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              <span>Borrow Request (Need Funds)</span>
            </button>
            <button
              type="button"
              onClick={() => set('listingType', 'lend')}
              className={`py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                form.listingType === 'lend'
                  ? 'bg-card text-blue-600 dark:text-blue-400 shadow-xs border border-blue-500/30 font-extrabold'
                  : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              <span>Lender Offer (Provide Capital)</span>
            </button>
          </div>
        </div>
        ...
        <div>
          <label className={labelCls}>
            {form.listingType === 'lend' ? 'Capital Available to Provide (₹) *' : 'Amount Needed (₹) *'}
          </label>
          <input
            type="number"
            min="1"
            ...
            placeholder={form.listingType === 'lend' ? 'Enter funding capital in ₹' : 'Enter loan amount in ₹'}
            className={inputCls}
          />
        </div>
            ...
            {LOAN_CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 rounded-full px-4 py-2.5 text-xs font-semibold border transition-all cursor-pointer ${isSelected
        ...
        <div>
          <label className={labelCls}>
            {form.listingType === 'lend' ? 'Offer Terms & Conditions *' : 'Description & Purpose *'}
          </label>
          <textarea
            rows={3}
            value={form.description}
            onChange={(e) => set('description', e.target.value)}
            placeholder={form.listingType === 'lend' ? 'Describe your lending criteria, eligible borrowers, preferred tenure…' : 'Enter description and purpose of loan…'}
            maxLength={2000}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={labelCls}>
              {form.listingType === 'lend' ? 'Offered Rate %' : 'Interest hint (%)'}
            </label>
            <input
              type="number"
              min="0"
          ...
          <div>
            <label className={labelCls}>
              {form.listingType === 'lend' ? 'Max Duration (days)' : 'Duration (days)'}
            </label>
            <input
              type="number"
              min="1"
